...
| ⭐ Emoji 消消乐 | 闯关制三消：限定步数内达到目标分过关（第 30 关难度到顶）；面板从 7×7 长到 9×10，带 💣 炸弹（相邻消除即引爆、炸掉周围 3×3 并震动棋盘）、💎 万能元素（可当任意 emoji 参与连线）与 🧱 不可消除的墙（emoji 可穿过下落） |
| 🗾 数独 | 唯一解挖洞生成，填错即标红并扣 ❤️ 生命，支持笔记候选，记录各难度最佳用时 |
| 🧱 Emoji 大师 | 羊了个羊闯关玩法：点击未被遮挡的顶层卡片落入 7 格收集槽，凑齐 3 张相同 emoji 自动消除；关卡越高 emoji 种类与层数越多（第 30 关到顶：32 种 / 8 层），洗牌每关限一次，失败可重玩本关 |
...
- 📊 三个闯关游戏（连连看 / 消消乐 / Emoji 大师）顶部卡片底部有本关进度条；操作区统一为「新游戏」按钮（二次确认后清记录、从第 1 关重来），失败时在结算浮层上「重玩本关」
