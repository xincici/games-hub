...
└── games/                # 每个游戏一个目录，utils 已扁平化到游戏目录内
    ...
    └── master/           # MasterGame.vue（Emoji 大师 / 羊了个羊闯关玩法：分层堆叠 + 遮挡判定、7 格收集槽三消、关卡难度曲线（关数越高 emoji 种类与层数越多）、洗牌每关限一次、计时、新游戏二次确认）+ board.js（纯逻辑：难度曲线 + 保证可解的分层发牌）+ i18n.js（route /master，key 前缀 __emoji_master__）
