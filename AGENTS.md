...
- **localStorage 约定**（各游戏互不干扰，前缀与原独立项目一致）：
  ...
  - Emoji 大师：`__emoji_master__*`（`__emoji_master__help_showed`，局面存档 `__emoji_master__state`（盘面分层卡片、收集槽、计时秒数、洗牌是否已用，胜利或失败后清除），最佳通关用时按前缀+难度数字存为 `__emoji_master__1`（单关卡））
