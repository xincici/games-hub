...
└── games/                # 每个游戏一个目录，utils 已扁平化到游戏目录内
    ...
    ├── link/             # LinkGame.vue（emoji 连连看 · 闯关制：限定时间内清空全盘过关，难度 = 列数 6→9（上限 9 列，10 列在手机上格子太小）+ 行数 6→12 + emoji 种类 6→12 + 对数 9→30 + 墙 0→18 + 限时，第 30 关全部封顶但关数无限；同一种 emoji 可出多对（偶数张即可）；顶部统计条底部有本关进度条（已消对数/总对数），操作区只有「新游戏」（点击弹二次确认、清记录回第 1 关），失败遮罩上给「重玩本关」；倒计时只显示在顶部统计条；棋盘宽度取「撑满可用宽度」与「放得下可用高度」的较小值（`boardStyle` 给 `--board-w`，CSS `width: min(100%, var(--board-w))`）——行数一多时按高度收窄，整盘始终完整可见，字号 `--font` 也按同一个宽度算，两者不会打架）+ board.js（纯逻辑：≤2 转弯路径查找、关卡曲线 CURVE 与 levelConfig、buildPairPool 多对发牌、generateLevelBoard/generateWalls、死局重排）+ i18n.js（route /link，key 前缀 __emoji_link__）
