...
- **localStorage 约定**（各游戏互不干扰，前缀与原独立项目一致）：
  ...
  - Emoji 侦探：`__emoji_detective__*`（关卡进度与局面同一个 key `__emoji_detective__level`（JSON：关卡、剩余❤️、阶段、用时、盘面、被偷换的下标），最高关卡 `__emoji_detective__best`；**胜利局面会连盘面带答案一起保留**，重进仍是结算层，玩家自己选「重玩本关 / 下一关」；中途退出（记忆 / 翻面 / 作答）不还原，直接重开当前关）
  - Emoji 猎手：`__emoji_hunter__*`（关卡进度与局面同一个 key `__emoji_hunter__level`（JSON：关卡、剩余❤️、阶段、用时、展示牌、候选牌、已找回与标错的 emoji），最高关卡 `__emoji_hunter__best`；**胜利局面连牌一起保留**，重进仍是结算层，玩家自己选「重玩本关 / 下一关」；中途退出直接重开当前关）
  - Threes：`__threes_game__*`（局面存档 `__threes_game__state`，最高分 `__threes_game__best`）
  - Emoji 消消乐：`__emoji_crush__*`（闯关进度 `__emoji_crush__level`=当前关卡，历史最高关卡 `__emoji_crush__best_1`（沿用「前缀+数字」以便连点标题清记录），局面存档 `__emoji_crush__state`（关卡、盘面（含墙/炸弹/万能元素的负值标记）、得分、剩余步数、胜负状态；**胜利局面保留**，退出重进仍是结算层、由玩家自己点「下一关」；失败清除）；旧的 `__emoji_crush__difficulty`、`__emoji_crush__best_2/3` 已废弃）
