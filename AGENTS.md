...
└── games/                # 每个游戏一个目录，utils 已扁平化到游戏目录内
    ...
    ├── hunter/           # HunterGame.vue（emoji 猎手：记忆→翻面→从候选区找回全部目标；选错候选扣 1 条命，生命行用 `shared/Hearts.vue`）+ i18n.js（route /hunter，key 前缀 __emoji_hunter__）
