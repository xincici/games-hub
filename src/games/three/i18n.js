export default {
  en: {
    gameTitle: 'Threes',
    bestScore: '🏆 Best Score',
    score: '✨ Score',
    nextLabel: '⏭️ Next',
    start: '🎮 New Game',
    tipLost: 'Game Over',
    helpTip: 'Click for help!',
    helpMsg: 'Swipe to slide all tiles. 1 and 2 merge into a 3; equal tiles of 3 and above merge into their double. One new tile spawns each move — plan ahead with the next-tile preview!',
    help1: '1 + 2 = 3; tiles 3 and above merge only with their equal: 3+3=6, 6+6=12 …',
    help2: 'Score is the sum of all tiles on the board: 1s and 2s are worth nothing, 3→3, 6→9, 12→27, 24→81 …',
    help3: 'The small number in a tile\'s corner is that tile\'s score.',
    help4: 'New tiles come from a shuffled deck of 1s, 2s and 3s; the next tile is previewed in the top bar.',
    confirmText: 'OK, I See',
    rotateTip: 'Please rotate your device to portrait'
  },
  cn: {
    gameTitle: 'Threes',
    bestScore: '🏆 最高分',
    score: '✨ 得分',
    nextLabel: '⏭️ 下一张',
    start: '🎮 新游戏',
    tipLost: '游戏结束',
    helpTip: '查看帮助',
    helpMsg: '滑动屏幕让所有牌一起移动：1 和 2 合成 3，3 及以上的相同数字两两合成双倍。每走一步会补进一张新牌——结合「下一张」预告提前规划！',
    help1: '1 + 2 = 3；3 及以上只能与相同数字合成：3+3=6、6+6=12……',
    help2: '得分是盘面上所有牌的分值之和：1 和 2 不计分，3→3 分、6→9 分、12→27 分、24→81 分……',
    help3: '每张牌右上角的小数字就是这张牌的分值。',
    help4: '新牌来自一叠洗好的 1、2、3 牌堆，顶部「下一张」随时预告下一张牌。',
    confirmText: '好的，明白',
    rotateTip: '请旋转设备至竖屏'
  }
};
