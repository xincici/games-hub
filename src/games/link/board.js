...
// 路径可借用棋盘外一圈虚拟空格，路径上不能有其他牌。

// 严格位于 a、b 之间的格子是否全部为空（a、b 自身不参与判断）
function lineClear(board, H, W, r1, c1, r2, c2) {
  if (r1 === r2) {
    const [from, to] = c1 < c2 ? [c1 + 1, c2 - 1] : [c2 + 1, c1 - 1];
    for (let c = from; c <= to; c++) {
      if (blocked(board, H, W, r1, c)) return false;
    }
    return true;
  }
  if (c1 === c2) {
    ...
    for (let r = from; r <= to; r++) {
      if (blocked(board, H, W, r, c1)) return false;
    }
    return true;
  }
  return false;
...
// 棋盘外一圈视为空格，不阻挡
function blocked(board, H, W, r, c) {
  if (r < 0 || r >= H || c < 0 || c >= W) return false;
  return board[r][c] != null;
}

function empty(board, H, W, r, c) {
  return !blocked(board, H, W, r, c);
}

// 查找 p1→p2 的连线（≤2 转折），返回途径点序列（含端点与拐点，
// 拐点坐标可能落在棋盘外一圈 -1..H / -1..W），不可达返回 null
export function findPath(board, H, W, r1, c1, r2, c2) {
  // 0 转折
  if ((r1 === r2 || c1 === c2) && lineClear(board, H, W, r1, c1, r2, c2)) {
    return [[r1, c1], [r2, c2]];
  }
  // 1 转折：经由 (r1,c2) 或 (r2,c1)
  for (const [cr, cc] of [[r1, c2], [r2, c1]]) {
    if (empty(board, H, W, cr, cc)
      && lineClear(board, H, W, r1, c1, cr, cc)
      && lineClear(board, H, W, cr, cc, r2, c2)) {
  ...
  for (let cc = -1; cc <= W; cc++) {
    ...
    if (empty(board, H, W, r1, cc) && empty(board, H, W, r2, cc)
      && lineClear(board, H, W, r1, c1, r1, cc)
      && lineClear(board, H, W, r1, cc, r2, cc)
      && lineClear(board, H, W, r2, cc, r2, c2)) {
      return [[r1, c1], [r1, cc], [r2, cc], [r2, c2]];
    }
  }
  for (let rr = -1; rr <= H; rr++) {
    if (rr === r1 || rr === r2) continue;
    if (empty(board, H, W, rr, c1) && empty(board, H, W, rr, c2)
      && lineClear(board, H, W, r1, c1, rr, c1)
      && lineClear(board, H, W, rr, c1, rr, c2)
      && lineClear(board, H, W, rr, c2, r2, c2)) {
...
export function generateSolvable(H, W, emojis, rand = Math.random, pairs = (H * W) / 2) {
  for (let attempt = 0; attempt < 100; attempt++) {
    const board = Array.from({ length: H }, () => Array.from({ length: W }, () => null));
    const emptyCells = [];
    for (let r = 0; r < H; r++) for (let c = 0; c < W; c++) emptyCells.push([r, c]);
    ...
    while (placed < pairs) {
      ...
      for (let a = 0; a < order.length; a++) {
        ...
        for (let b = a + 1; b < order.length; b++) {
          ...
          if (findPath(board, H, W, r1, c1, r2, c2)) {
  ...
  const solution = [];
  let placed = 0;
  for (let r = 0; r < H && placed < pairs; r++) {
    for (let c = 0; c + 1 < W && placed < pairs; c += 2) {
...
export function generateBoard(H, W, emojis, rand, pairs) {
  return generateSolvable(H, W, emojis, rand, pairs).board;
}

// 死局检测：盘面上是否还存在一对可连通的同 emoji 牌
export function hasMove(board, H, W) {
  ...
  for (const positions of byEmoji.values()) {
    for (let i = 0; i < positions.length; i++) {
      for (let j = i + 1; j < positions.length; j++) {
        ...
        if (findPath(board, H, W, r1, c1, r2, c2)) return true;
      }
    }
  }
  return false;
...
export function shuffleBoard(board, H, W, rand = Math.random) {
  const positions = [];
  const emojis = [];
  for (let r = 0; r < H; r++) {
    for (let c = 0; c < W; c++) {
  ...
  for (let attempt = 0; attempt < 200; attempt++) {
    ...
    if (hasMove(next, H, W)) return next;
  }
  return board;
}
